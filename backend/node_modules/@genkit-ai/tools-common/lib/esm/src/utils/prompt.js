...
function renderMessages(messages) {
    ...
    messages.forEach((message) => {
        ...
        const hasSupportedPart = message.content.length === 0 ||
            message.content.some((p) => 'text' in p || 'media' in p);
        const hasUnsupportedPart = message.content.some((p) => !('text' in p) && !('media' in p));
...
function cleanupFrontmatter(frontmatter) {
    return recursiveCleanup(frontmatter) || {};
}
function recursiveCleanup(val) {
...
function partToString(part) {
    ...
    const type = Object.keys(part).find((k) => k !== 'metadata' && part[k] !== undefined) || 'unknown';
