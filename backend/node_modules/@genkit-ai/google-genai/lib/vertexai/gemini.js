...
const GeminiConfigSchema = import_model.GenerationCommonConfigSchema.extend({
  ...
  payGo: import_genkit.z.enum(["priority", "priority-only", "flex", "flex-only"]).describe(
    'PayGo consumption options. Priority provides more consistent performance than Standard PayGo. Flex is a cost-effective option for non-critical workloads. The "-only" options use only PayGo and no Provisioned Throughput.'
  ).optional(),
...
const GeminiImageConfigSchema = GeminiConfigSchema.extend({
  imageConfig: import_genkit.z.object({
    ...
    imageSize: import_genkit.z.enum(["512", "1K", "2K", "4K"]).optional()
  }).passthrough().optional()
}).passthrough();
...
function listActions(models) {
  ...
  return models.filter(
    (m) => (isGeminiModelName((0, import_utils2.modelName)(m.name)) || isImageModelName((0, import_utils2.modelName)(m.name))) && !KNOWN_DECOMISSIONED_MODELS.includes((0, import_utils2.modelName)(m.name) || "")
  ).map((m) => {
...
function defineModel(name, clientOptions, pluginOptions) {
  ...
  return (0, import_plugin.model)(
    ...
    async (request, { streamingRequested, sendChunk, abortSignal }) => {
      ...
      clientOpt = (0, import_utils2.calculateRequestOptions)(clientOpt, {
        location,
        apiKey: apiKeyFromConfig
      });
      if (payGo) {
      ...
      const modelVersion = versionFromConfig || (0, import_utils2.extractVersion)(ref);
      ...
      if (jsonMode && request.output?.constrained) {
        if (pluginOptions?.legacyResponseSchema) {
          generateContentRequest.generationConfig.responseSchema = (0, import_utils2.cleanSchema)(
