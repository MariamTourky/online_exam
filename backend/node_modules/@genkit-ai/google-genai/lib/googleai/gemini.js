...
const MAX_INLINE_MEDIA_BYTES = 1024 * 1024 * 100;
...
const GeminiConfigSchema = import_model.GenerationCommonConfigSchema.extend({
  ...
  urlContext: import_genkit.z.union([import_genkit.z.boolean(), import_genkit.z.object({}).passthrough()]).describe("Return grounding metadata from links included in the query").optional(),
  ...
  }).passthrough().describe("Configuration for retrieval tools.").optional(),
  ...
  serviceTier: import_genkit.z.union([import_genkit.z.enum(["standard", "flex", "priority"]), import_genkit.z.string()]).describe("Service tier for the Gemini API.").optional(),
  thinkingConfig: import_genkit.z.object({
    ...
    thinkingLevel: import_genkit.z.enum(["MINIMAL", "LOW", "MEDIUM", "HIGH"]).describe(
...
const GeminiImageConfigSchema = GeminiConfigSchema.extend({
  ...
  google_search: import_genkit.z.object({
    searchTypes: import_genkit.z.object({
      webSearch: import_genkit.z.object({}).optional(),
      imageSearch: import_genkit.z.object({}).optional()
    }).optional()
  }).describe(
    "Retrieve public web data for grounding, powered by Google Search."
  ).passthrough().optional()
...
function isGeminiModelName(value) {
  return value.startsWith("gemini-") && !isTTSModelName(value) && !isImageModelName(value);
}
...
const KNOWN_GEMMA_MODELS = {
  ...
  "gemma-4-31b-it": commonRef("gemma-4-31b-it", void 0, GemmaConfigSchema),
...
function defineModel(name, pluginOptions) {
        ...
        filter: (part) => {
          try {
            ...
            if (
              ...
              return false;
            if (supportsExternalUrls && (url.protocol === "https:" || url.protocol === "http:")) {
              return false;
            }
          } catch {
          }
  ...
  return (0, import_plugin.model)(
    ...
    async (request, { streamingRequested, sendChunk, abortSignal }) => {
      ...
      const modelVersion = request.config?.version || (0, import_utils2.extractVersion)(ref);
      ...
      if (toolConfigConfig) {
        if (Object.hasOwnProperty.call(
          ...
          toolConfigConfig["include_server_side_tool_invocations"] = toolConfigConfig["includeServerSideToolInvocations"];
      ...
      const generationConfig = {
        ...(0, import_utils2.removeClientOptionOverrides)(restOfConfigOptions),
      ...
      if (request.output?.constrained && jsonMode) {
        if (pluginOptions?.legacyResponseSchema) {
          generationConfig.responseSchema = (0, import_utils2.cleanSchema)(request.output.schema);
        } else {
      ...
      const generateApiKey = (0, import_utils2.calculateApiKey)(
